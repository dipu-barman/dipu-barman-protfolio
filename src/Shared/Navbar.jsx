...
export default function Navbar() {
  ...
  // Logo component: Icon + Gradient text
  const Logo = () => (
    <motion.div
      className="flex items-center gap-2 cursor-pointer select-none"
      onClick={() => scrollToSection("home")}
      whileHover={{ scale: 1.1, color: "#14B8A6" }} // Teal-500 hover color
      whileTap={{ scale: 0.95 }}
      style={{ userSelect: "none" }}
    >
      <FaCode size={28} className="text-indigo-500" />
      <span className="text-transparent bg-clip-text bg-gradient-to-r bg-gradient-to-r from-red-900 via-cyan-500 to-red-500
  ...
  return (
    <div
      ...
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        scrolled
          ? "bg-gradient-to-r from-indigo-600 via-teal-500 to-indigo-600 shadow-lg border-b border-indigo-700"
                ...
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 transition-all ${
                    activeItem === item.id
                      ...
                      : "text-gray-200 hover:text-white hover:bg-indigo-900/50"
                  }`}
            ...
            <motion.button
              ...
              className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 transition-all bg-red-600 ${
                activeItem === contactItem.id
                  ? "text-white bg-red-600/90 shadow-md"
                  : "text-gray-100 hover:text-white hover:bg-indigo-900/50"
              }`}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden bg-gradient-to-r from-indigo-600 via-teal-500 to-indigo-600 border-t border-indigo-700"
          >
